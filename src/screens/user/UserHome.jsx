...
import { BookOpen, Radio, ChevronRight, Star, Clock, PlayCircle, Video } from "lucide-react";
  ...
  return (
    <section className="uh-fade" style={{ marginTop: 36, animationDelay: `${delay}ms` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 19, fontWeight: 800, color: colors.user.text }}>
  ...
  return (
    <div style={{ width: "100%", maxWidth: 1200, margin: "0 auto", boxSizing: "border-box" }}>
      ...
      >
        ...
        <h2 style={{ margin: 0, fontSize: 30, fontWeight: 900, maxWidth: 540, letterSpacing: -0.5, position: "relative", color: "#FFFFFF" }}>{hero.title}</h2>
        <p style={{ margin: "10px 0 22px", opacity: 0.85, fontSize: 15.5, maxWidth: 460, position: "relative", color: "#FFFFFF" }}>{hero.subtitle}</p>
              ...
              return (
                <div key={en.id || c.id} className="uh-card" onClick={() => navigate(`/app/player/${c.id}`)} style={{ display: "flex" }}>
                  <div style={{ width: 116, flexShrink: 0, position: "relative", overflow: "hidden" }}>
                    <div className="uh-thumb" style={{ position: "absolute", inset: 0, background: c.thumbnail ? `url(${c.thumbnail}) center/cover` : colors.gradients.heroWarm }} />
                    <BookOpen size={30} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", color: "rgba(255,255,255,0.92)", filter: "drop-shadow(0 2px 8px rgba(0,0,0,0.5))" }} />
                  </div>
                  <div style={{ padding: "14px 16px", flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 800, fontSize: 14.5, lineHeight: 1.35, color: colors.user.text, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{c.title}</div>
                    <div style={{ display: "flex", justifyContent: "space-between", color: colors.user.subHeading, fontSize: 12, margin: "8px 0 6px" }}>
                      ...
                      <span style={{ color: colors.user.accent, fontWeight: 700 }}>Resume →</span>
                    </div>
            ...
            {experts.map((e) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 10, position: "relative" }}>
                  ...
                  {!!e.is_available && (
                    <span className="uh-online-dot" style={{ position: "absolute", bottom: 2, right: "calc(50% - 30px)", width: 13, height: 13, borderRadius: "50%", background: "#22C55E", border: `2.5px solid ${colors.user.card}` }} />
                  )}
                </div>
                <div style={{ fontWeight: 800, fontSize: 14, color: colors.user.text }}>{e.user?.name || e.name}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 9, fontSize: 12, color: colors.user.subHeading }}>
                  ...
                  <span style={{ color: colors.user.accent, fontWeight: 800 }}>₹{e.video_rate || 0}/min</span>
                </div>
              </div>
            ))}
          </div>
              ...
              return (
                <div key={c.id} className="uh-card" onClick={() => navigate(`/app/course/${c.id}`)}>
                  <div style={{ height: 136, position: "relative", overflow: "hidden" }}>
                    <div className="uh-thumb" style={{ position: "absolute", inset: 0, background: c.thumbnail ? `url(${c.thumbnail}) center/cover` : colors.gradients.heroWarm, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {!c.thumbnail && <BookOpen size={34} color="rgba(255,255,255,0.55)" />}
                    </div>
                    {c.level && (
                      <span style={{ position: "absolute", top: 10, left: 10, background: "rgba(8,12,37,0.7)", backdropFilter: "blur(4px)", padding: "3px 10px", borderRadius: 99, fontSize: 11, fontWeight: 700, color: "#FFFFFF" }}>{c.level}</span>
                    )}
                    <span style={{ position: "absolute", bottom: 10, right: 10, background: Number(c.price) > 0 ? "rgba(8,12,37,0.75)" : "rgba(16,185,129,0.85)", backdropFilter: "blur(4px)", padding: "4px 12px", borderRadius: 99, fontSize: 12.5, fontWeight: 900, color: "#FFFFFF" }}>
                      {Number(c.price) > 0 ? formatCurrency(c.price) : "Free"}
                    </span>
                  </div>
                  <div style={{ padding: 14 }}>
                    <div style={{ fontWeight: 800, fontSize: 14.5, lineHeight: 1.35, minHeight: 39, color: colors.user.text, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{c.title}</div>
      ...
      <Section title="Upcoming Webinars" onSeeAll={() => navigate("/app/explore?tab=webinars")} delay={240}>
        {loading ? (
          ...
          <div className="uh-card" style={{ cursor: "default", padding: 30, textAlign: "center", background: colors.gradients.heroWarm, border: "none" }}>
            <div style={{ fontSize: 30, marginBottom: 8 }}>📡</div>
            <div style={{ fontWeight: 800, fontSize: 16.5, color: "#FFFFFF" }}>Webinars Coming Soon!</div>
            <div style={{ opacity: 0.75, fontSize: 13.5, marginTop: 4, color: "#FFFFFF" }}>Live sessions from creators will appear here.</div>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 16 }}>
            {webinars.map((w) => (
              <div key={w.id} className="uh-card" onClick={() => navigate(`/app/webinar/${w.id}`)}>
                <div style={{ height: 136, position: "relative", overflow: "hidden" }}>
                  <div className="uh-thumb" style={{ position: "absolute", inset: 0, background: w.thumbnail ? `url(${w.thumbnail}) center/cover` : colors.gradients.heroWarm, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {!w.thumbnail && <Video size={34} color="rgba(255,255,255,0.55)" />}
                  </div>
                  <span style={{ position: "absolute", top: 10, left: 10, background: isToday(w.scheduled_at) ? "rgba(239,68,68,0.9)" : "rgba(8,12,37,0.7)", backdropFilter: "blur(4px)", padding: "4px 11px", borderRadius: 99, fontSize: 11.5, fontWeight: 800, color: "#FFFFFF", display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <span style={{ position: "absolute", bottom: 10, right: 10, background: Number(w.price) > 0 ? "rgba(8,12,37,0.75)" : "rgba(16,185,129,0.85)", backdropFilter: "blur(4px)", padding: "4px 12px", borderRadius: 99, fontSize: 12.5, fontWeight: 900, color: "#FFFFFF" }}>
                    {Number(w.price) > 0 ? formatCurrency(w.price) : "Free"}
                  </span>
                </div>
                <div style={{ padding: 14 }}>
                  <div style={{ fontWeight: 800, fontSize: 14.5, lineHeight: 1.35, minHeight: 39, color: colors.user.text, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{w.title}</div>
                  <div style={{ color: colors.user.subHeading, fontSize: 12.5, marginTop: 6 }}>by {w.creator?.name || "Creator"}</div>
                  <div style={{ display: "flex", gap: 12, marginTop: 9, color: colors.user.subHeading, fontSize: 12 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      <Video size={12} />
                      {w.scheduled_at ? new Date(w.scheduled_at).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "Live Stream"}
                    </span>
                  </div>
                </div>
              </div>
